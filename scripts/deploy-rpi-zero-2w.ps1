...
if ($PSCmdlet.ShouldProcess("$($config.User)@$($config.Host):$($config.RemoteDir)", "Copy trekr binary and launch script")) {
    Copy-RemoteFiles -Config $config -Paths @($artifactPath, $launchScriptPath)
}
...
$remoteFinalize = "chmod +x $remoteTrekr $remoteLauncher"
