...
- `cargo run -- capture-ui --state-mode demo --capture-rect 460,220,220,120 --capture-padding 8,12,24,12` applies per-edge padding (left,top,right,bottom) to explicit rect crops
- `powershell -ExecutionPolicy Bypass -File .\scripts\capture-ui-hotspots.ps1 -Hotspot transport-left,transport-right,status-strip,timeline-header-controls,fx-row -Zoom 8` captures review hotspots and emits nearest-neighbor zooms
- `powershell -ExecutionPolicy Bypass -File .\scripts\capture-ui-hotspot-ab.ps1 -BaselineLabel baseline -CandidateLabel candidate -Zoom 8` captures paired baseline/candidate hotspot sets for A/B review
...
- `--capture-padding` accepts either a uniform pixel value (`--capture-padding 16`) or per-edge values (`--capture-padding left,top,right,bottom`) and works with both `--capture-region` and `--capture-rect` (applied after region/rect resolution, clamped to image bounds)
...
- `scripts/capture-ui-screens.ps1` now also supports `-Theme`, `-CaptureZoom`, and `-SkipBuild`; when `-CaptureZoom` is >1 it emits nearest-neighbor `@Nx` PNGs and annotates the manifest with zoom metadata
- `scripts/capture-ui-hotspots.ps1` captures a small preset hotspot set or named subset, writes `hotspots-manifest.json`, and emits nearest-neighbor zoom PNGs for pixel review
- `scripts/capture-ui-hotspot-ab.ps1` captures paired baseline/candidate hotspot sets into sibling folders and writes `comparison-manifest.json` for A/B inspection
...
- `scripts/capture-ui-screens.ps1`: asks `trekr` itself to render `timeline`, `timeline-focused`, `mappings`, `midi-io`, and `routing` screenshots into `artifacts/screenshots`
  ...
  - supports scripted capture options (`-Theme`, `-Script`, `-Sequence`, `-CaptureRegion`, `-CaptureRect`, `-CapturePadding`, `-CaptureZoom`, `-Annotate`) and forwards them to `capture-ui`
...
- `scripts/capture-ui-hotspots.ps1`: captures review-oriented hotspot crops such as `transport-left`, `transport-right`, `status-strip`, `timeline-header-controls`, and `fx-row`, plus nearest-neighbor zoom images
- `scripts/capture-ui-hotspot-ab.ps1`: captures paired baseline/candidate hotspot sets and emits a comparison manifest for A/B review
- `scripts/review-ui-screens.ps1`: calls `codex exec` with those screenshots attached and writes findings to `artifacts/reviews/ui-findings.md`
  - `-Recurse` can include nested baseline/candidate hotspot folders in one review pass
