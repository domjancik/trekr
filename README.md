...
- `cargo run -- run --state-mode demo --video-mode kmsdrm-console --preview-stream mjpeg:0.0.0.0:8090` also exposes an app-owned OBS preview stream at `http://<pi-ip>:8090/`
...
- `--preview-stream <mjpeg:host:port>` enables an app-owned MJPEG HTTP preview server for interactive runs only
- `--preview-fps <n>` changes the preview capture cadence for interactive runs only; default is `12`
...
- `TREKR_PREVIEW_STREAM=0.0.0.0:8090` and `TREKR_PREVIEW_FPS=12` provide environment defaults for the preview server when the flags are omitted
...
Pi OBS preview stream:

- launch `trekr` on the Pi with `cargo run -- run --state-mode demo --video-mode kmsdrm-console --preview-stream mjpeg:0.0.0.0:8090`
- optionally set `--preview-fps 10` or `TREKR_PREVIEW_FPS=10` if the Pi needs a lighter capture cadence
- open `http://<pi-ip>:8090/healthz` from another machine if you want a simple readiness check
- for OBS, use **Browser Source** and point it at `http://<pi-ip>:8090/`; the root page is a minimal HTML wrapper around the live `/preview.mjpg` feed
- the direct MJPEG endpoint remains available at `http://<pi-ip>:8090/preview.mjpg` for testing or tools that can ingest MJPEG directly
- this stream is generated by `trekr` itself, so it avoids the `ffmpeg kmsgrab` / DRM-master contention that blocks external KMSDRM capture while the app is running
